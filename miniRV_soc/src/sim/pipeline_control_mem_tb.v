// ============================================================================
// pipeline_control_mem_tb.v — 流水线控制信号传播 + 访存测试
// ============================================================================
// 测试:
//   1. LUI, AUIPC, JALR — 控制信号经流水线寄存器正确传播
//   2. SB/LB/LBU, SH/LH/LHU — 字节/半字访存和符号扩展
//   3. JAL 跳转冲刷
// ============================================================================
...
module pipeline_control_mem_tb;
    ...
    initial begin
        ...
        imem[ 3] = enc_i(0,  3, 3'b000, 4, 7'b1100111);    // jalr x4,0(x3) → PC=64

        imem[16] = enc_i(-128, 0, 3'b000, 5, 7'b0010011);  // addi x5,x0,-128
        imem[17] = enc_s(1, 5, 0, 3'b000);                 // sb x5,1(x0)   ← mem[0].byte1 = 0x80
        imem[18] = enc_i(1, 0, 3'b100, 6, 7'b0000011);     // lbu x6,1(x0)  ← x6 = 0x80
        imem[19] = enc_i(1, 0, 3'b000, 7, 7'b0000011);     // lb x7,1(x0)   ← x7 = -128
        imem[20] = enc_i(-2, 0, 3'b000, 8, 7'b0010011);    // addi x8,x0,-2
        imem[21] = enc_s(2, 8, 0, 3'b001);                 // sh x8,2(x0)   ← mem[0].half1 = 0xfffe
        imem[22] = enc_i(2, 0, 3'b101, 9, 7'b0000011);     // lhu x9,2(x0)  ← x9 = 0xfffe
        imem[23] = enc_i(2, 0, 3'b001, 10, 7'b0000011);    // lh x10,2(x0)  ← x10 = -2
        imem[24] = enc_j(8, 11);                           // jal x11,+8
        imem[25] = enc_i(99, 0, 3'b000, 12, 7'b0010011);   // ← 被冲刷
        imem[26] = enc_i(12, 0, 3'b000, 12, 7'b0010011);   // 跳转目标: x12=12
