# ============================================================================
# create_ips.tcl -- Batch generate Vivado IPs for miniRV_soc
# ============================================================================
# Usage (from miniRV_soc directory):
...
# ---------------------------------------------------------------------------
# Add ip_dir as an IP repository so Vivado recognizes the IPs
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. Add IPs to project fileset (needed for generate_target in batch)
# ---------------------------------------------------------------------------
set ip_files [list]
foreach ip [get_ips] {
    set ip_name [get_property NAME $ip]
    set xci_file "$ip_dir/${ip_name}/${ip_name}.xci"
    if {[file exists $xci_file]} {
        lappend ip_files $xci_file
        add_files -norecurse $xci_file
        set_property used_in_synthesis true [get_files $xci_file]
    }
}

# ---------------------------------------------------------------------------
# 9. Generate wrappers (.veo) - now that IPs are in project
# ---------------------------------------------------------------------------
puts ""
